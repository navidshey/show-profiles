...
return (
   <>
    {isLoading && <Spinner/>}
    {profiles && profiles.results.map((character)=>{
     return (<ProfileCard character={character}/>);
    })}
    {!isLoading && !profiles && <div>Error in fetching data!</div>}
    {profiles && profiles.info && +profiles.info.count>0 && <Pagination prop={profiles.info}></Pagination>}
    </>
 );
}
